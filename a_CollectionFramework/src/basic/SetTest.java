...
public class SetTest {
	...
	public static void main(String[] args) {
		...
		System.out.println("Set 데이터수 > " + hs1.size());
		System.out.println();
		
		//Set은 데이터의 순서가 없기 때문에 List 처럼 인덱스로 데이터를 하나씩 불러올 수 없다. 그래서 데이터를 하나씩 얻기 위해서는 Iterator로 변환해야 한다
		//Set의 데이터를 Iterator로 변환하기 > Set의 Iterator()메서드를 호출하면 된다
		Iterator it = hs1.iterator();
		
		//데이터 개수만큼 반복하기
		//hasNext()메서드 > 포인터를 다음 위치에 옮기고, 데이터가 있으면 true, 없으면 false 반환	
		while(it.hasNext()) { //다음 자료가 있는지 검사
			//next() 메서드 > 포인터를 다음자료 위치로 이동하고, 이동한 자료를 반환
