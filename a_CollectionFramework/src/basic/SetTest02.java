...
public class SetTest02 {
	...
	public static void main(String[] args) {
		...
		// > headSet(기준갑, 논리값) (논리값이 true이면 '기준값'을 포함 시킴)
		SortedSet<String> ss1 = ts.headSet("K");
		System.out.println();
		System.out.println("headSet 발동");
		...
		System.out.println();
		System.out.println("tailSet 발동");
		System.out.println("K 이후 자료 > " + ss2);
		System.out.println("K 이후 자료(기준값 포함) > " + ts.tailSet("K", true));
		
		//subSet(기준값1, 기준값2) > 기준값1 ~ 기준값2 사이의 값을 가져온다
		//('기준값1' 포함, '기준값2' 미포함)
		//subSet(기준값1, 논리값1, 기준값2, 논리값2) > 각 기준값을 포함할지 여부를 결정한다(논리값이 true이면 포함, false이면 미포함)
		System.out.println();
		System.out.println("subSet 발동");
		System.out.println("K(포함)부터 N(미포함)까지 > " + ts.subSet("K", "N"));
		System.out.println("K(true) ~ N(ture) > " + ts.subSet("K", true, "N", true));
		System.out.println("K(false) ~ N(false) > " + ts.subSet("K", false, "N", false));
		System.out.println("K(false ~ N(ture) > " + ts.subSet("K", false, "N", true));
